...
foreach ($idTarefas as $id) {
    ...
    if ($modo == "finalizar") {
        $sql = "UPDATE cadastro_usuarios.tarefa SET finalizada = true WHERE id = $id and id_usuario = $idUsuario";
    } else if ($modo == "excluir") {
        $sql = "DELETE FROM cadastro_usuarios.tarefa WHERE id = $id and id_usuario = $idUsuario";
    }
